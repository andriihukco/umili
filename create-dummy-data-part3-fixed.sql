-- Umili Freelance Marketplace - Dummy Data Part 3 (Fixed)
...
-- First, let's get the task IDs and conversation IDs
-- We'll use a more direct approach to avoid subquery issues
...
-- Task ID: (SELECT id FROM public.tasks WHERE title = 'Розробка платформи для онлайн-курсів' LIMIT 1)
-- Conversation ID: (SELECT id FROM public.conversations WHERE task_id = task_id AND client_id = '77777777-7777-7777-7777-777777777777' AND freelancer_id = '55555555-5555-5555-5555-555555555555' LIMIT 1)

-- We'll insert messages one by one to avoid subquery issues
-- First, let's get the conversation ID for the online courses platform
...
-- ✅ Messages for conversations with proper JOIN queries
...
-- 4. Run this script (create-dummy-data-part3-fixed.sql) fourth
